...
import { User as UserIcon, Mail, Lock, ArrowRight, Loader2, MapPin } from 'lucide-react';
...
export default function Signup({ onNavigate, onLogin }: SignupProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!location) {
            setError('Location access is mandatory to create an account for security reasons.');
            return;
        }
        ...
        try {
            const user = await authService.signup(name, email, password, location);
            onLogin(user);
        } catch (err) {
        ...
        <div className="min-h-[calc(100vh-4rem)] bg-gradient-mesh flex items-center justify-center p-4 transition-colors duration-300 relative overflow-hidden">
            ...
            <div className="bg-[var(--bg-secondary)]/80 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-md p-8 border border-[var(--border-color)] relative overflow-hidden">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-[var(--text-secondary)] text-sm">Join CARIVIX for intelligent data insights</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <div className="relative group">
                            <UserIcon className="w-5 h-5 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2 group-focus-within:text-blue-600 transition-colors" />
                            <input
                                type="text"
                                required
                                value={name}
